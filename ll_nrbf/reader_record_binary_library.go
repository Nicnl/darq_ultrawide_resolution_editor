...
type RecordBinaryLibrary struct {
	LibraryId   int32
	LibraryName string
}
...
func (d *Decoder) decodeRecordBinaryLibrary() (rbl RecordBinaryLibrary, err error) {
	// LibraryId, must be positive
	err = binary.Read(d.r, binary.LittleEndian, &rbl.LibraryId)
	if err != nil {
		return
	}
